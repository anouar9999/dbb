...
const Modal = ({ isOpen, onClose, profile, onStatusUpdate }) => {
  ...
  const fetchUserDetails = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}get_user_details.php?user_id=${profile.id}`);
  ...
  const handleStatusUpdate = useCallback(
    async (status) => {
      try {
        const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}update_registration_status.php`, {
  ...
  const fetchProfiles = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}participants_registration.php?tournament_id=${tournamentId}`,
      );
      if (response.data.success) {
