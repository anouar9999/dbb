...
const UserManagement = () => {
  ...
  const fetchUsers = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}manage_users.php`);
  ...
  const handleSaveUser = async (editedUser) => {
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}manage_users.php`, editedUser);
      if (response.data.success) {
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        const response = await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}manage_users.php`, {
          data: { id: userId },
